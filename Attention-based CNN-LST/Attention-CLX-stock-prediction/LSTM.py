...
# Enable eager execution for TensorFlow/Keras compatibility
...
print(model.summary())
# Create separate optimizers for each model to avoid variable conflicts
...
plt.legend()
plt.savefig('./results/lstm/residuals_training_validation_loss.png', dpi=300, bbox_inches='tight')
plt.close()
...
plt.legend()
plt.savefig('./results/lstm/lstm_training_validation_loss.png', dpi=300, bbox_inches='tight')
plt.close()

# Save training histories
history_df = pd.DataFrame(history.history)
history_df.to_csv('./results/lstm/residuals_training_history.csv', index=False)
yuan_history_df = pd.DataFrame(yuan_history.history)
yuan_history_df.to_csv('./results/lstm/lstm_training_history.csv', index=False)
...
plt.legend()
plt.savefig('./results/lstm/bilstm_stock_price_prediction.png', dpi=300, bbox_inches='tight')
plt.close()

# Save BiLSTM predictions
finalpredicted_stock_price.to_csv('./results/lstm/bilstm_predictions.csv')
...
plt.ylabel('Close', fontsize=14, horizontalalignment='center')
plt.legend()
plt.savefig('./results/lstm/lstm_stock_price_prediction.png', dpi=300, bbox_inches='tight')
plt.close()

# Save LSTM predictions
yuan_predicted_stock_price1.to_csv('./results/lstm/lstm_predictions.csv')
...
# Save metrics
with open('./results/lstm/metrics.txt', 'w') as f:
    f.write("BiLSTM Model Evaluation Metrics\n")
    f.write("="*50 + "\n")
    MSE = metrics.mean_squared_error(finalpredicted_stock_price['close'], yhat)
    RMSE = MSE**0.5
    MAE = metrics.mean_absolute_error(finalpredicted_stock_price['close'], yhat)
    R2 = metrics.r2_score(finalpredicted_stock_price['close'], yhat)
    f.write(f'MSE: {MSE:.5f}\n')
    f.write(f'RMSE: {RMSE:.5f}\n')
    f.write(f'MAE: {MAE:.5f}\n')
    f.write(f'R2: {R2:.5f}\n')

print("\n=== LSTM Experiments Completed ===")
print("Results saved in ./results/lstm/")
